
<?php include_once PATH . '/core/include.php'; ?>
<?php include_once "core/Model/indexModel.php"; ?>
<?php include_once "mod_ajuda/Model/indexModel.php"; ?>
<!-- =============== HEADER HTML PAGE ================= -->
<?php include_once "template/page/headerPage.php"; ?>
<!-- =================== HEADER ============================ -->
<?php include_once "template/page/header.php"; ?>
<!-- =================== MENU  ============================ -->
<?php include_once "template/page/menu.php"; ?>
<!-- =================== CORPO  ============================ -->
<?php include_once "template/page/corpoHeader.php"; ?>

<a class="btn btn-success" href="<?= FuncaoBase::geraLink("ajuda", "este", "index") ?>">Voltar</a>
<a class="btn btn-info" href="<?= FuncaoBase::geraLink("ajuda", "este", "cadastro") ?>" title="Novo Registro">+ Novo</a>

<br>
<br>

<form method="post" action="#" name="frmBuscaEste" id="frmBuscaEste">
    <label>Pesquisa :</label>
    <input type="text" class="form form-control" name="searchEsteName" id="searcEsteName">

    <br>

    <input type="submit" class="btn btn-info" name="btnBuscaEste" id="btnBuscaEste" value="Pesquisar">

</form>

<?php


$btn = isset($_POST['btnBuscaEste']) ? $_POST['btnBuscaEste'] : null;
$nome = isset($_POST['searchEsteName']) ? $_POST['searchEsteName'] : null;

if ($btn == 'Pesquisar') {

    $busca = new EsteConEstoqueModel();
    $estes = $busca->listaNome($nome);
    
    //var_dump($unidades);

    print "<legend>Pesquisa Este</legend>";

    print "<div class=\"table-responsive\"><table class=\"table table-bordered table-striped\">
    <thead>
            <tr>
                <th>idteste</th>
<th>data</th>
<th>CodUf</th>
<th>Codmundv</th>
<th>Codmun</th>
<th>NomeMunic</th>
<th>Opções</th>
            </tr>
</thead>
<tbody>";

    foreach ($estes as $este) {

            print "<tr>
                    <td>".$este['idteste']."</td>
<td>".$este['data']."</td>
<td>".$este['CodUf']."</td>
<td>".$este['Codmundv']."</td>
<td>".$este['Codmun']."</td>
<td>".$este['NomeMunic']."</td>
";
                    
            print "<td>";
            print "<a href='" . FuncaoBase::geraLink("ajuda", "este", "view", array('id' => $este['idteste'])) . "'><img src='/core/imagem/view.png' title='Visualizar Registro'></a>|";
            print "<a href='" . FuncaoBase::geraLink("ajuda", "este", "edit", array('id' => $este['idteste'])) . "'><img src='/core/imagem/editar.png' title='Editar Registro'></a>|";
            print "<a href='" . FuncaoBase::geraLink("ajuda", "este", "delete", array('id' => $este['idteste'])) . "' onclick=\"return confirm('Deseja Deletar esse Registro ?')\"><img src='/core/imagem/delete.png' title='Deletar Registro'></a>";

            print
                    "</td>";

            print "</tr>";
        }
       

    print " </tbody></table></div>";
}
?>


<br>
<!-- =================== RODAPE CORPO ==================== -->
<?php include_once "template/page/corpoRodape.php"; ?>
<!-- =================== RODAPE  ======================== -->
<?php include_once "template/page/rodape.php" ?>
<?php include_once "template/page/barra_config_template.php"; ?>
<!-- =============== HEADER HTML PAGE ================= -->
<?php include_once "template/page/rodapePage.php"; ?>
<script>

    $(document).ready(function () {

        var itens = {
            data:
<?php print json_encode($dadosEste); ?>, // array com os dados
            getValue: "nome", /* alterar com nome do item BD */
            list: {
                match: {
                    enabled: true
                },

                onSelectItemEvent: function () {
                    //var id = $("#searcid_marca").getSelectedItemData().id_marca;
                    //var nome = $("#searcid_marca").getSelectedItemData().nome;

                    // $("#nomeMarca_fk").val(nome); // Mudar
                    //$("#id_marca").val(id);
                }
            }
        };
        /*********** autocomplete ***********/
        $("#searchEsteName").easyAutocomplete(itens);

    });
</script>
        